...
int main()
{
    ...
    off.hoodPivot = glm::vec3(0.0f, 3.00f, 0.65f);   // bisagra un poco más baja y atrás
    off.hoodLocal = glm::vec3(2.70f, 0.35f, 4.55f);  // asentado del cofre
    ...
    spotLights[2] = SpotLight(
        0.25f, 0.6f, 1.0f,     // color azul
        0.1f, 6.0f,            // ambient, diffuse (más brillo)
        0.0f, 0.0f, 0.0f,      // pos (dummy, se actualiza)
        0.0f, 0.0f, 1.0f,      // dir (dummy, se actualiza)
        1.0f, 0.009f, 0.00032f,// atenuación de largo alcance
        16.0f                  // cono focal
    );
    spotLightCount++;
    ...
    while (!mainWindow.getShouldClose())
    {
        ...
        // === INPUT: abrir/cerrar cofre ===
        {
            ...
            const float hoodSpeed = 50.0f; // grados/seg
            if (k[GLFW_KEY_T]) car.openHood(+hoodSpeed * deltaTime); // abrir
            if (k[GLFW_KEY_G]) car.openHood(-hoodSpeed * deltaTime); // cerrar
        }
        ...
        {
            const bool* k = mainWindow.getsKeys();
            const float nud = 0.1f;   // traslación fina
            ...
            // Parabrisas
            if (k[GLFW_KEY_J]) off.wsLocal.x -= nud; // izquierda
            if (k[GLFW_KEY_L]) off.wsLocal.x += nud; // derecha
            if (k[GLFW_KEY_I]) off.wsLocal.y += nud; // arriba
            if (k[GLFW_KEY_K]) off.wsLocal.y -= nud; // abajo
            if (k[GLFW_KEY_O]) off.wsLocal.z += nud; // adelante local
            if (k[GLFW_KEY_P]) off.wsLocal.z -= nud; // atrás local
